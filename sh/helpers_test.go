...
func compareFiles(file1 string, file2 string) error {
	...
	if err != nil {
		return mg.WrapError(err, fmt.Errorf("can't stat %s: %v", file1, err))
	}
	s2, err := os.Stat(file2)
	if err != nil {
		return mg.WrapError(err, fmt.Errorf("can't stat %s: %v", file2, err))
	}
	if s1.Size() != s2.Size() {
	...
	if err != nil {
		return mg.WrapError(err, fmt.Errorf("can't read %s: %v", file1, err))
	}
	f2bytes, err := ioutil.ReadFile(file2)
	if err != nil {
		return mg.WrapError(err, fmt.Errorf("can't read %s: %v", file2, err))
	}
